#本文件提供了一套简单的防御动作，以蛇形机动和爬升为主要途径，仅供参考使用
#使用前在本文件的基础上进行修改，避免出现bug
...
TurnMode=[0,0,0,0]#用于给每架飞机确定转向的顺逆时针
def DefenseAction(output_cmd,info,DroneID,plane_Yaw,ob,mp):
   ...
   #信息获取
    ...
    RDer=RD()
    MissleDirectionList=KU.GetMissleDirection(info,DroneID,plane_Yaw)#记录导弹的真实航向
    RelativeList=KU.GetRelative(info,DroneID)#记录导弹相对于本机的角度
    EnemyList=[]
    #获取敌机位置避免偷袭
   ...
   #局势判断       
    ...
    if DefenseStage[int((DroneID/100000)-1)]==0:
        ...
        if DefenseMode[int((DroneID/100000)-1)]==0:
            ...
            output_cmd.sPlaneControl.CmdSpd = 1.5
            output_cmd.sPlaneControl.CmdPhi = 60
            output_cmd.sPlaneControl.CmdNy = 8
            output_cmd.sPlaneControl.isApplyNow = True
            output_cmd.sPlaneControl.CmdThrust = 500
            output_cmd.sPlaneControl.ThrustLimit = 500
            DefenseDeg[int((DroneID/100000)-1)]=MissleDirectionList[0]
            if RelativeList[0]>0 : #此处需要确定探测瞬间的导弹角度，并在后续不再更改，否则飞机将原地盘旋，请结合实际情况添加语句
                TurnMode[int((DroneID/100000)-1)] = -1
            elif RelativeList[0]<0 :
                TurnMode[int((DroneID/100000)-1)] = 1
            output_cmd.sPlaneControl.TurnDirection = TurnMode[int((DroneID/100000)-1)]
            output_cmd.sPlaneControl.CmdHeadingDeg = DefenseDeg[int((DroneID/100000)-1)]
            if abs(RDer.superr2d(info.Yaw)-MissleDirectionList[0])<10:               
                DefenseDeg[int((DroneID/100000)-1)]=RDer.superr2d(info.Yaw)
            ...
            output_cmd.sPlaneControl.CmdThrust = 500
            output_cmd.sPlaneControl.ThrustLimit = 500
            #此处需要确定探测瞬间的导弹角度，并在后续不再更改，否则飞机将原地盘旋，请结合实际情况添加语句
            if RelativeList[0]>0 :
                TurnMode[int((DroneID/100000)-1)] = -1
                DefenseDeg[int((DroneID/100000)-1)]=(RDer.superr2d(info.Yaw)+175)%360
            elif RelativeList[0]<0 :
                TurnMode[int((DroneID/100000)-1)] = 1
                DefenseDeg[int((DroneID/100000)-1)]=(RDer.superr2d(info.Yaw)-175)%360
            output_cmd.sPlaneControl.TurnDirection = TurnMode[int((DroneID/100000)-1)]
            output_cmd.sPlaneControl.CmdHeadingDeg = DefenseDeg[int((DroneID/100000)-1)]
            if abs(RDer.superr2d(info.Yaw)-DefenseDeg[int((DroneID/100000)-1)])<10:
    ...
    #防御阶段1，执行躲避机动动作            
        ...
        if DefenseMode[int((DroneID/100000)-1)]==0:#蛇形机动躲避
            JDDZer.SheXing(45,DefenseDeg[int((DroneID/100000)-1)]+40,DefenseDeg[int((DroneID/100000)-1)]-40,4,1.2,140)
            
        elif DefenseMode[int((DroneID/100000)-1)]==1:#最速爬升躲避
            JDDZer.PaSheng(DefenseDeg[int((DroneID/100000)-1)],3.0, DefenseAlt[int((DroneID/100000)-1)],400,200)
